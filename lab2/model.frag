...
{   
    ...
    vec3 pointColor = CalcPointLight(pointLight, norm, FragPos, viewDir);
    vec4 pointResult = vec4 (pointColor, 1.0);

    FragColor = dirColor + pointResult;
...
vec4 CalcDirLight(DirLight light, vec3 normal, vec3 viewDir)
{
    ...
    return (a+d+s) * texture(texture_diffuse1, TexCoords);
}
